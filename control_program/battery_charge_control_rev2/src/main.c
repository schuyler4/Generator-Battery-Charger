...
#define BATTERY_CURRENT_ADC_CODE_SET_POINT 100
...
float compare_match_command = 150;
...
void control_charger(void)
{
    ...
    {
        compare_match_command += (BATTERY_CURRENT_ADC_CODE_SET_POINT - average_current_adc_value)*0.01;
    }
    else if(range_max && average_current_adc_value > BATTERY_CURRENT_ADC_CODE_SET_POINT && in_rpm_range)
    {
        compare_match_command -= (average_current_adc_value - BATTERY_CURRENT_ADC_CODE_SET_POINT)*0.01;
    }
}
